        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <?php include 'topbar.php'; 
                ...
                if(isset($_POST['submit'])){
                    ...
                    $query = "UPDATE kriteria SET nama_kriteria='$nama_kriteria', bobot='$bobot', jenis_kriteria='$jenis_kriteria' WHERE id_kriteria='".$_REQUEST['id_kriteria']."'";
                    $result = $mysqli->query($query);
                    if($result){
                        echo "<div class='row'>";
                                echo "<div class='col-sm-9'></div>";
                                echo "<div class='col-sm-3 text-right alert alert-success alert-dismissible fade show'>Kriteria Berhasil Diubah!</div>";
                                echo "</div>";
                            echo "</div>";
                    }else{
                        echo "<script>alert('Data gagal diubah'); window.location='kriteria.php'</script>";
                    }
                }

                $query = "SELECT * FROM kriteria WHERE id_kriteria='".$mysqli->real_escape_string($_REQUEST['id_kriteria'])."'limit 0,1";
                $result = $mysqli->query($query);
                                ...
                                <div class="card-body">
                                    <form method="POST" action="#" enctype="multipart/form-data">
                                        <div class="form-group">
                                            ...
                                            <input type="text" value='<?php echo $nama_kriteria; ?>' name="nama_kriteria" id="nama_kriteria" required="required" placeholder="ketik" autocomplete="off" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label for="bobot">Bobot</label>
                                            <input type="number" value='<?php echo $bobot; ?>' name="bobot" id="bobot" required="required" placeholder="ketik" autocomplete="off" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label for="Jenis Kriteria">Jenis Kriteria</label>
                                            
                                            <select name = 'jenis_kriteria' class="form-control">
                                                <option value = 'Benefit' <?php if($jenis_kriteria=='Benefit') echo 'selected'?> >Benefit</option>
                                                <option value = 'Cost' <?php if($jenis_kriteria=='Cost') echo 'selected'?>>Cost</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
